...
public class PMultiCond extends PCond<PField_Not> implements PMultiBase<PCond<?>> {
    ...
    public String getText(PCLCardTarget perspective, Object requestor, boolean addPeriod) {
        ...
        return amount > 0 ? (capital(TEXT.act_choose(getAmountRawString(requestor)), addPeriod) + COLON_SEPARATOR + base) : base;
    }
    ...
    public void triggerOnAllyWithdraw(PCLCard c, PCLCardAlly ally, boolean triggerEffects) {
        for (PSkill<?> effect : effects) {
            effect.triggerOnAllyWithdraw(c, ally, triggerEffects);
        }
    }
