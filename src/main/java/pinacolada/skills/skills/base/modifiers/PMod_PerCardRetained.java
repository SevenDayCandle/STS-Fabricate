...
@VisibleSkill
public class PMod_PerCardRetained extends PMod_PerCardHas {
    public static final PSkillData<PField_CardCategory> DATA = register(PMod_PerCardRetained.class, PField_CardCategory.class)
            .setOrigins(PCLCardSelection.Manual)
            .setDestinations(PCLCardSelection.Manual)
            .setGroups(PCLCardGroupHelper.Hand)
            .noTarget();

    public PMod_PerCardRetained() {
        this(1, 0);
    }

    public PMod_PerCardRetained(int amount) {
        this(amount, 0);
    }

    public PMod_PerCardRetained(int amount, int extra) {
        super(DATA, amount, extra, PCLCardGroupHelper.Hand);
    }

    public PMod_PerCardRetained(PSkillSaveData content) {
        super(DATA, content);
    }
