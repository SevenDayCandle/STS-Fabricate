...
@VisibleSkill
public class PCond_HaveLastRetained extends PCond_HaveLastCard {
    public static final PSkillData<PField_CardCategory> DATA = register(PCond_HaveLastRetained.class, PField_CardCategory.class)
            .setOrigins(PCLCardSelection.Manual)
            .setDestinations(PCLCardSelection.Manual)
            .setGroups(PCLCardGroupHelper.Hand)
            .noTarget();

    public PCond_HaveLastRetained() {
        this(1);
    }

    public PCond_HaveLastRetained(int amount) {
        super(DATA, amount, PCLCardGroupHelper.Hand);
    }

    public PCond_HaveLastRetained(PSkillSaveData content) {
        super(DATA, content);
    }
