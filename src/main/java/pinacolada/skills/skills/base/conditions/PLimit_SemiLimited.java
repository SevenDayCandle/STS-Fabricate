...
@VisibleSkill
public class PLimit_SemiLimited extends PCond<PField_Empty> {
    ...
    public boolean tryPassParent(PSkill<?> source, PCLUseInfo info) {
        return info.tryActivateSemiLimited() && super.tryPassParent(source, info);
    }

    @Override
    public void use(PCLUseInfo info, PCLActions order) {
        if (info.tryActivateSemiLimited() && childEffect != null) {
    ...
    public void use(PCLUseInfo info, PCLActions order, boolean shouldPay) {
        if ((shouldPay ? info.tryActivateSemiLimited() : checkCondition(info, true, null)) && childEffect != null) {
            childEffect.use(info, order);
        }
    }
}
