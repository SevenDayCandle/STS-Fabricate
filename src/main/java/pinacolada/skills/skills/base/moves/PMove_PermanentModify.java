...
public abstract class PMove_PermanentModify extends PMove_Modify<PField_CardModify> implements OutOfCombatMove {
    ...
    public ActionT1<AbstractCard> getAction(PCLActions order) {
        return (c) -> {
            order.add(modifyCard(c, amount, fields.forced, !fields.not, fields.or));
            AbstractCard deckCopy = GameUtilities.getMasterDeckInstance(c.uuid);
            if (deckCopy != null && deckCopy != c) {
                order.add(modifyCard(deckCopy, amount, fields.forced, !fields.not, fields.or));
            }
        };
    }
        ...
        if (list != null) {
            PCLEffects.Queue.callback(() -> {
                        for (AbstractCard c : list) {
                            applyModifierOutsideOfCombat(c, amount);
                        }
                    })
                    .addCallback(() -> {
